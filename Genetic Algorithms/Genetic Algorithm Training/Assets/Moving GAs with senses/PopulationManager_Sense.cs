...
public class PopulationManager_Sense : MonoBehaviour {
    ...
    void Start () {
        for (int i = 0; i < populationSize; i++)
        {
            ...
            GameObject g = Instantiate(agentPrefab, randomPosition, Quaternion.identity);

            g.GetComponent<Brain_Sense>().Init();
    ...
    {
        List<GameObject> sortedPopulation = population.OrderBy(p => p.GetComponent<Brain_Sense>().timeAlive + 
                                                                    p.GetComponent<Brain_Sense>().timeTravelled * 2).ToList();
        population.Clear();

        for(int i = (int)(sortedPopulation.Count / 2.0f) -1 ; i < sortedPopulation.Count - 1; i++)
        {
            Breed(sortedPopulation[i].GetComponent<Brain_Sense>().dna, sortedPopulation[i + 1].GetComponent<Brain_Sense>().dna);
            Breed(sortedPopulation[i + 1].GetComponent<Brain_Sense>().dna, sortedPopulation[i].GetComponent<Brain_Sense>().dna);
        }
